...
class ChatAPIView(APIView):
    ...
    def query_patients(self, query_params=None, user=None):
        ...
        try:
            ...
            return self.permission_handler.filter_data_by_permission(user, 'patients', patient_data)
    ...
    def query_appointments(self, query_params=None, user=None):
        ...
        try:
            ...
            # Filter based on permissions
            return self.permission_handler.filter_data_by_permission(user, 'appointments', appointment_data)
    ...
    def query_medications(self, query_params=None, user=None):
        ...
        try:
            ...
            # Filter based on permissions
            return self.permission_handler.filter_data_by_permission(user, 'medications', medication_data)
    ...
    def query_doctors(self, query_params=None, user=None):
        ...
        try:
            ...
            # Filter based on permissions
            return self.permission_handler.filter_data_by_permission(user, 'doctors', doctor_data)
    ...
    def process_medical_request(self, message: str, user) -> Dict:
        ...
        try:
            ...
            if re.search(r'analyze.*report|report.*analysis', message.lower()):
                ...
                report_text = re.sub(r'analyze.*report|report.*analysis', '', message, flags=re.IGNORECASE).strip()
                if report_text:
            ...
            if re.search(r'visualize.*data|data.*visualization', message.lower()):
                ...
                if data_match:
                    try:
                        ...
                        visualization = self.medical_analysis.generate_medical_visualization(data)
                        return {
                            "success": True,
                            "type": "visualization",
            ...
            return {
                "success": False,
                "error": "Could not process medical request"
            }
        except Exception as e:
            logger.error(f"Error processing medical request: {str(e)}")
            return {
                "success": False,
                "error": str(e)
            }
