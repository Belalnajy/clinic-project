...
class PatientSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if 'insurance_expiration_date' in data and data['insurance_expiration_date']:
            if data['insurance_expiration_date'] < timezone.now().date():
                raise serializers.ValidationError("Insurance expiration date cannot be in the past")

        # Validate height and weight if both are provided
        if 'height' in data and 'weight' in data and data['height'] is not None and data['weight'] is not None:
            ...
            if height > 0 and weight > 0:
                ...
                if bmi < 10 or bmi > 60:
                    raise serializers.ValidationError("Invalid height and weight combination")
    ...
    def validate_email(self, value):
        ...
        """
        if value and Patient.objects.filter(email=value).exclude(pk=self.instance.pk if self.instance else None).exists():
...
class EmergencyContactSerializer(serializers.ModelSerializer):
    ...
    def validate_phone_number(self, value):
        ...
        Validate phone number format
        """
        if value and EmergencyContact.objects.filter(phone_number=value).exclude(pk=self.instance.pk if self.instance else None).exists():
            raise serializers.ValidationError("An emergency contact with this phone number already exists")
        return value

    def validate(self, data):
        """
        Custom validation for emergency contact data
        """
        # Validate relationship is not empty
        if 'relationship' in data and not data['relationship'].strip():
            raise serializers.ValidationError("Relationship cannot be empty")

        # Validate at least one contact method is provided
        if not data.get('phone_number'):
            raise serializers.ValidationError("At least one contact method (phone number) is required")
